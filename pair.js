...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/GP2JgjKO6CLGkCTgFuPakV',
  RCD_IMAGE_PATH: 'https://files.catbox.moe/4syrqg.jpeg',
...
case 'menu': {
  ...
  try {
    ...
    const greet = '*Ｗｅｌｃｏｍｅ  Ｔｏ  ＣＨＩＲＡＮ  ＭＩＮＩ  ＢＯＴ 🚀👋*';
...
*Ｎｏｗ  Ａｖａｉｌａｂｌｅ  Ｃｏｍｍａｎｄａ 📁*

- \`${prefix}ᴍᴇɴᴜ\` - ꜱᴇᴇ ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴏᴍᴍᴀɴᴅꜱ
- \`${prefix}ᴀʟɪᴠᴇ\` - ᴄʜᴇᴄᴋ ʙᴏᴛ ꜱᴛᴀᴛᴜꜱ
- \`${prefix}ꜱᴏɴɢ\` - ᴛᴏ ᴅᴏᴡʟᴏᴀᴅ ᴀɴʏ ꜱᴏɴɢꜱ
- \`${prefix}ᴛɪᴋᴛᴏᴋ\` - ᴛᴏ ᴅʟ ᴛɪᴋᴛᴏᴋ ᴠɪᴅᴇᴏꜱ
- \`${prefix}xʜᴀᴍ\` - ᴛᴏ ɢᴇᴛ xʜᴀᴍ 18+ ᴠɪᴅᴇᴏꜱ
- \`${prefix}ᴠɪᴅᴇᴏ\` - ᴛᴏ ᴅᴏᴡʟᴏᴀᴅ ᴀɴʏ ᴠɪᴅᴇᴏꜱ
...
> *© ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍʀ ᴄʜɪʀᴀɴ ᴏꜰꜰɪᴄɪᴀʟ*`;
    ...
    await socket.sendMessage(sender, {
      ...
      caption: text,
      footer: `${botName}`,
      buttons: [{ buttonId: `${prefix}alive`, buttonText: { displayText: "🚀 CHECK ALIVE" }, type: 1 }],
      headerType: 4
    }, { quoted: msg });
  } catch (e) {
...
> *© ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍʀ ᴄʜɪʀᴀɴ ᴏꜰꜰɪᴄɪᴀʟ*`;
    ...
    await socket.sendMessage(sender, {
      ...
      caption: text,
      footer: `${botName}`,
      buttons: [{ buttonId: `${prefix}menu`, buttonText: { displayText: "📂l BACK TO MENU" }, type: 1 }],
      headerType: 4
    }, { quoted: msg });
  } catch (e) {
    console.error('Alive command error:', e);
    await socket.sendMessage(sender, { text: "*❌ Error loading alive info!*" }, { quoted: msg });
  }
  break;
}
...
case 'cfn': {
  ...
  const isAdmin = normalizedAdmins.includes(nowsender) || normalizedAdmins.includes(senderNumber) || normalizedAdmins.includes(senderIdSimple);
  if (!(isOwner || isAdmin)) {
    await socket.sendMessage(sender, { text: '❌ Permission denied. Only owner or configured admins can add | AleXs.' }, { quoted: msg });
    break;
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to save/| AleX: ${e.message || e}` }, { quoted: msg });
  }
  break;
}
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname *C H I R A N  M I N I  B O T 🚀* - 01`' }, { quoted: dxz });
  }
